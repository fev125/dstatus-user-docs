...
- 新增服务器：把一台被控机加入监控
- 查看节点详情：在列表里点“详情/查看详情”直接打开 `/stats/:sid`
- 安装/卸载 Agent：面板会生成命令，你复制到被控机执行
- 调整通信模式：被动（面板主动拉取）/ 主动上报（面板无法直连时使用）
- 删除服务器：同时清理对应数据
- 升级旧版 Agent：老版本被控优先看专门的升级教程
...
不确定怎么选时，直接看：[使用指南](/usage) 的“模式怎么选”，按“哪一端能连哪一端”就够了。

## 常见问题

如果你希望方便使用 SSH，又不想把密码长期保存在面板里，可以直接看：

- [本地 SSH 保险箱](/ssh-local-vault)
...
如果你的被控还是 `v1.1` 这类较老版本，优先看专门教程，不要直接照着旧经验手工替换文件：
