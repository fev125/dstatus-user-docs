...
```text
请先查看用户文档 https://docs.vps.mom/mcp，再按教程把我的 DStatus 远程 MCP 接上：https://你的实例域名/mcp
```

你只负责复制、发送、点允许。**下面「给 AI」是给 Agent 看的。**

## 给 AI

读完本页后，帮用户接上远程 MCP（用户会给出 `https://…/mcp` 地址）：

1. 以 Streamable HTTP 把该地址加入本机 MCP 列表。  
2. 优先 OAuth：随机本地端口做回调，**先监听再打开浏览器**，用户点允许后写入 credentials。  
3. 验收：握手成功并能列出工具；不要复述 token。  
4. 不支持 OAuth 时，再引导用户用面板生成 Token。
