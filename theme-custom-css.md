...
**纪律三**：想要**整站暗黑风格**（纯黑、赛博、BMW 这类），只改 `--dark-*` 一侧，然后把站点默认主题切到夜间模式用。不要把 `--light-*` 写成深色——图表坐标轴等组件按"当前模式"选色，日间模式刷黑后它们照常输出深色文字，会花。另外强调色别用纯白/纯黑，按钮文字会看不见。

状态色不分日夜，直接改：`--success-color`（绿）、`--warning-color`（黄橙）、`--error-color`（红）、`--info-color`（蓝紫）。改它们会连带徽章、提示框一起变——这是设计如此，不要再单独改徽章。
