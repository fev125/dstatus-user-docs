...
需要手动下载 Agent 二进制文件，配置 `config.yaml`，然后手动启动。详细步骤请参考：
...
- 确认通信端口已开放（默认 9999）
- 检查防火墙规则
- 验证 `config.yaml` 中的 `key` 配置正确

**主动模式**：
- 确认 `config.yaml` 中的 `report_server` 地址正确
- 检查 `report_server_key` 与面板配置的密钥一致
- 确认 `server_id` 填写正确（节点 SID）
